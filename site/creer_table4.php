...
<head>
	<meta charset ="utf-8">
	<title> Etape 2B- Table Acteur</title>
...
<div class="card">
	<div class="card-header">
		<h4><span class='d-inline-block bg-info-sql text-white px-2 py-1'>2B</span> Créer table "realisateur"</h4>
	</div>
...
<div class="mb-3">
		<span class='d-inline-block bg-info-sql-l text-white px-2 py-1'>a</span> <strong>TABLE realisateur </strong> </br>

</div>
	
		<h6 id="acteur">Créer la table "realisateur" :</h6>

avec  :	
		<ul>
		<li><strong>id_rea:</strong> identifiant unique qui est utilisé comme clé primaire et qui n’est pas nulle et (peut être auto incrémentée à chaque nouvel enregistrement).</li>
		...
		<li><strong>age   :</strong> age du réalisateur type de donnée à détermine</li>
		</ul>	
		  ...
		  <div class="col-md-6 mb-3 " >
			...
			<?php 
					if(isset($_POST['execute_req1'])){
									$exec = $films-> create_table3($_POST['req_sql1'],'realisateur');
					}
					elseif (isset($_POST['execute_req3'])){
						
						//$exec = $films-> sup_table2('acteur');
					}
					?>
...
<div class="card-body">
	...
	<div class="col">
		...
		<?php foreach ($tables as $table ) {
			  ...
			  <?php foreach ($_tables as $_table){?>
					<tr>	
						<td><?php 
						$val = $films -> verif_schema($table['TABLE_NAME'],$_table['COLUMN_NAME']);
						 if ($val) {?><img src="images/icon/v.JPG" width="20px"> <?php } else {?>
                                                              <img src="images/icon/f.JPG" width="20px"><?php $refaire= TRUE; }?></td>
						<th><?php echo $_table['COLUMN_NAME'];?></th>
						<td><?php 
						$val = $films -> verif_schema2($table['TABLE_NAME'],$_table['COLUMN_NAME'],'COLUMN_TYPE',$_table['COLUMN_TYPE']);
                                                if ($val) {?><img src="images/icon/v.JPG" width="20px"> <?php } else {?>
						...
						<td><?php 
						$val = $films -> verif_schema2($table['TABLE_NAME'],$_table['COLUMN_NAME'],'IS_NULLABLE',$_table['IS_NULLABLE']);
                                                if ($val) {?><img src="images/icon/v.JPG" width="20px"> <?php } else {?>
                                                              <img src="images/icon/f.JPG" width="20px"><?php $refaire= TRUE;}?></td>
						<td><?php echo $_table['IS_NULLABLE'];?></td>
						<td><?php 
						$val = $films -> verif_schema2($table['TABLE_NAME'],$_table['COLUMN_NAME'],'COLUMN_DEFAULT',$_table['COLUMN_DEFAULT']);
                                                if ($val) {?><img src="images/icon/v.JPG" width="20px"> <?php } else {?>
                                                              <img src="images/icon/f.JPG" width="20px"><?php $refaire= TRUE;}?></td>
						<td><?php echo $_table['COLUMN_DEFAULT'];?></td>
						<td><?php 
						$val = $films -> verif_schema2($table['TABLE_NAME'],$_table['COLUMN_NAME'],'EXTRA',$_table['EXTRA']);
                                                if ($val) {?><img src="images/icon/v.JPG" width="20px"> <?php } else {?>
                                                              <img src="images/icon/f.JPG" width="20px"><?php $refaire= TRUE; }?></td>
						<td><?php echo $_table['EXTRA'];?></td>
						<td><?php 
						$val = $films -> verif_schema2($table['TABLE_NAME'],$_table['COLUMN_NAME'],'COLUMN_KEY',$_table['COLUMN_KEY']);
                                                if ($val) {?><img src="images/icon/v.JPG" width="20px"> <?php } else {?>
                                                              <img src="images/icon/f.JPG" width="20px"><?php $refaire= TRUE;}?></td>
		...
		<?php }?>
	<a class="btn btn-primary" href="creer_table3.php" role="button">Activité suivante : table et clef étrangère --> </a>
	</div>
</div>
